	...
	public partial class FormDefaultChangeContainer : XtraForm
	{
		...
		private void LoadLocationAndSizeSettings()
		{
			try
			{
				...
				int x = this.AppContext.UserSettings.GetValue<int>(UserSettings.SettingFormDefaultChangeContainerWindowLocationX, defaultValue: (Screen.PrimaryScreen!.Bounds.Width - this.Width) / 2);
				int y = this.AppContext.UserSettings.GetValue<int>(UserSettings.SettingFormDefaultChangeContainerWindowLocationY, defaultValue: (Screen.PrimaryScreen.Bounds.Height - this.Height) / 2);

				this.Location = new Point(x, y);

				// Set size.
				int width = this.AppContext.UserSettings.GetValue<int>(UserSettings.SettingFormDefaultChangeContainerWindowWidth, defaultValue: this.Size.Width);
				int height = this.AppContext.UserSettings.GetValue<int>(UserSettings.SettingFormDefaultChangeContainerWindowHeight, defaultValue: this.Size.Height);
		...
		private void OnObjectManager_PropertyValueChange(ChangePropertyValuePertyModelSimpleObjectChangeContainerContextRequesterEventArgs e)
		{
			TreeListNode node;
			...
			if (this.nodesBySimpleObject.TryGetValue(e.SimpleObject, out node))
			{
				...
				TreeListNode? propertyNode = (e.PropertyModel != null) ? this.FindPropertyNode(node, e.PropertyModel.PropertyIndex) : null;

				if (propertyNode == null && e.IsChanged)
				{
					this.AddPropertyNode(node, e.SimpleObject, e.PropertyModel!, e.PropertyValue);
				}
				else if (e.IsChanged) // e.SimpleObject.GetChangedPropertyIndexes().Contains(e.PropertyModel.Index))
				{
					this.RefreshPropertyNode(propertyNode!, e.SimpleObject, e.PropertyModel!, e.PropertyValue);
					...
					if (e.PropertyModel!.IsRelationTableId) // and if RelationObjectId do exists within changed properties, refresh objectIdPropertyNode and related object
					{
						...
						{
							TreeListNode? objectIdPropertyNode = this.FindPropertyNode(node, oneToOneOrManyRelationModel.PrimaryObjectIdPropertyModel.PropertyIndex); // this Index should be e.PropertyModel.Index + 1);

							if (objectIdPropertyNode != null)
								this.RefreshPropertyNode(objectIdPropertyNode, e.SimpleObject, oneToOneOrManyRelationModel.PrimaryObjectIdPropertyModel, e.PropertyValue);
						}
					}
				}
				...
				if (!(e.SimpleObject is GraphElement) && e.SimpleObject.GetModel().NamePropertyModel != null && e.PropertyModel != null && e.PropertyModel.PropertyIndex == e.SimpleObject.GetModel().NamePropertyModel.PropertyIndex)
				{
		...
		private void OnObjectManager_RelationForeignObjectSet(RelationForeignObjectSetChangeContainerContextRequesterEventArgs e)
		{
		...
		{
			imageName ??= simpleObject.GetImageName();
		...
		private void RefreshPropertyNode(TreeListNode propertyNode, SimpleObject simpleObject, IPropertyModel propertyModel, object? value)
		{
			...
			if (propertyModel.IsRelationObjectId)
			{
				...
				if (relationModel is IOneToOneOrManyRelationModel oneToOneOrManyRelationModel)
				{
					SimpleObject relatedSimpleObject = simpleObject.GetRelationPrimaryObject(oneToOneOrManyRelationModel);
					...
					{
						relatedObjectText += String.Format(" => {0} {1}", (oneToOneOrManyRelationModel).PrimaryObjectName, relatedSimpleObject.ValueToString()); // "null";
					}
				}
			}
