...
namespace Simple.Objects.ServerMonitor
{
	public partial class FormMain : SimpleRibbonFormBase
	{
		...
		public FormMain()
		{
			...
			//this.recentLabelItem2.Caption = "Copyright © 2023 Simple.Objects™. ALL RIGHTS RESERVED."; // AssemblyInfo.AssemblyCopyright;
		...
		{
			if (this.AppContext.UserSettings.RibbonSkinMaskColor != 0)
				UserLookAndFeel.Default.SkinMaskColor = Color.FromArgb(this.AppContext.UserSettings.RibbonSkinMaskColor);

			if (this.AppContext.UserSettings.RibbonSkinMaskColor2 != 0)
		...
		private void AppClient_OnSessionConnected(SessionConnectedMessageArgs e)
		{
			...
			SessionInfoRow row = new SessionInfoRow(rowIndex, e.SessionKey, e.ServerAddress!, e.ServerPort, e.ClientAddress!, e.ClientPort,
													username: String.Empty, connected: true, e.CharacterEncoding, context: this);
		...
		private void AppClient_BrodcastMessageSent(MessageMessageArgs e)
		{
			if (this.InvokeRequired)
		...
		private void AppClient_OnBrodcastMessageSent(MessageMessageArgs e)
		{
		...
		protected virtual PackageArgsFactory CreatePackageArgsFactory() => new PackageArgsFactory(new List<Assembly>() { this.GetType().Assembly, System.Reflection.Assembly.Load("Simple.Objects.ServerMonitor") });
			...
			if (response.ResponseSucceeded && response.SessionInfos != null)
			{
				for (long i = 0; i < response.SessionInfos.LongLength; i++)
				{
					...
					SessionInfoRow sessionInfoRow = new SessionInfoRow((int)i, sessionInfo.SessionKey, sessionInfo.ServerAddress, sessionInfo.ServerPort, sessionInfo.ClientAddress, sessionInfo.ClientPort,
																	   sessionInfo.Username, connected: true, sessionInfo.CharacterEncoding, context: this);
		...
		{
			FormSettings formSettings = new FormSettings(this.Ribbon);
		...
		private async ValueTask LoadUserSessions()
		{
			...
			try
			{
				...
				foreach (var item in dataSourceUserSessions)
					this.userSessionInfoRowsBySessionKey.Add(item.RowIndex, item);
