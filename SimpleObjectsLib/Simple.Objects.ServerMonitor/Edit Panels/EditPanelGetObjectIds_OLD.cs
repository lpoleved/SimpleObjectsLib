...
{
	//[SystemRequestArgs((int)SystemRequest.GetObjectIds)]
	public partial class EditPanelRequestGetObjectIds_OLD : EditPanelSessionPackageRequestResponse
	{
		private int columnObjectKey;

		public EditPanelRequestGetObjectIds_OLD()
		{
			InitializeComponent();
		...
		protected override void OnRefreshBindingObject()
		{
			...
			try
			{
				if (this.PackageInfoRow?.RequestOrMessagePackageInfo.PackageArgs is TableIdRequestArgs requestArgs)
				{
					...
					var objectModel = this.GetServerObjectModel(tableId);
