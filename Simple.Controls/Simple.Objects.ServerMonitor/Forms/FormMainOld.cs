...
{
	public partial class FormMainOld : RibbonForm, IMonitorSessionContext, IFormMain
	{
		...
		public FormMainOld()
		{
			...
			this.backstageViewControl.Visible = false;
			//base.Initialize();
			...
			//InitSchemeCombo();
			//InitPrint();
			//this.Text = DXperience.Demos.DemoHelper.GetFormText("SimplePad(C# Demo)");
			UserLookAndFeel.Default.SetSkinStyle("WXI");
			//Icon = DevExpress.Utils.ResourceImageHelper.CreateIconFromResourcesEx("DevExpress.XtraBars.Demos.RibbonSimplePad.AppIcon.ico", typeof(FormMainNew).Assembly);
		...
		void OnRibbonMessageClosed(object sender, RibbonMessageClosedArgs e)
		{
			if (e.Message == WhatsNewMessage && e.Result == DialogResult.OK)
				DevExpress.Data.Utils.SafeProcess.Start("https://www.netakod-community");
		...
		private void InitSchemeCombo()
		{
			...
			this.Ribbon.GetController().Changed += OnRibbonControllerChanged;
		}

		private void OnRibbonControllerChanged(object? sender, EventArgs e)
		{
			if (this.Ribbon.GetController().LookAndFeel.ActiveSkinName != "Office 2016 Colorful")
		...
		{
			pc = new PopupColorNew(container, this);
		}
		...
		{
			pmMain.ShowPopup(this.Ribbon.Manager, MousePosition);
		}
		#endregion
		...
		//	barSubItemChangeSkin.DropDownControl = skins;
		//	//CreateNewDocument();
		//	barEditItem1.EditValue = (Bitmap)DevExpress.Utils.ResourceImageHelper.CreateImageFromResources("DevExpress.XtraBars.Demos.RibbonSimplePad.online.gif", typeof(FormMainNew).Assembly);
		//}
		...
		private void simpleButtonExit_Click(object sender, EventArgs e) => this.Close();
		...
		{
			this.Ribbon.DeactivateKeyboardNavigation();
			pmAppMain.HidePopup();
			this.Refresh();
		...
		private void biStyle_EditValueChanged(object sender, EventArgs e)
		{
			...
			this.Ribbon.ApplicationButtonDropDownControl = style == RibbonControlStyle.Office2007 ? (object)pmAppMain : this.backstageViewControl;
			//if (style == RibbonControlStyle.TabletOffice || style == RibbonControlStyle.OfficeUniversal)
			//{
			//	this.barToggleSwitchItemAutoSave.Visibility = BarItemVisibility.Always;
			//}
			//else
			//{
			//	this.barToggleSwitchItemAutoSave.Visibility = BarItemVisibility.Never;
			//}
			UpdateLookAndFeel();
		...
		{
			string skinName;
			RibbonControlStyle style = this.Ribbon.RibbonStyle;
		...
		void InitPrintingSystem()
		{
			...
			BarManager manager = this.Ribbon.Manager;
		...
		void UpdatePrintPageOrientation(GalleryItem item)
		{
			...
			if (ddbOrientation.DropDownControl != null && this.printControl2.PrintingSystem != null)
		...
		{
			this.Ribbon.ColorScheme = ((RibbonControlColorScheme)e.Item.Value);
		}
		...
		private void barButtonItemColorMix_ItemClick(object sender, ItemClickEventArgs e)
		{
			ColorWheelForm form = new ColorWheelForm();
